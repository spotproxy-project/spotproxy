...
def simulate_curl(destination_url, test_duration=10):
    """
    Simulate traffic similar to 'curl' by executing HTTP GET requests.

    :param destination_url: The destination URL (e.g., "http://google.com").
    :param test_duration: Duration to simulate the traffic in seconds.
    """

    try:
        # Perform a curl-like operation using Python's subprocess
        hostname = destination_url.split("//")[-1].split("/")[0]

# Perform DNS resolution (UDP traffic)
...
#            print(f"Resolved {hostname} to {ip_address}")
        ...
        # Log the status
        print(
            f"Received response: {response.status_code}, Length: {len(response.content)}")

        # Pause for a short duration before the next request
        sleep(2)
        ...
        print(
            f"Simulation completed. Duration: {time() - start_time:.2f} seconds")


def get_and_set_wireguard_ip():
    try:
        response = requests.get(
            f'http://{CONTROLLER_IP_ADDRESS}:8000/assignments/getnew')
        if response.status_code == 200:
            ...
            if 'Address' in config:
                config = re.sub(r'Address = .*',
                                f'Address = {wireguard_ip}/32', config)
            else:
